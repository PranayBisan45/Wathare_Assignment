...
intialTime = new Date();
intialTime.setHours(15,00,00)
finalTime = new Date();
finalTime.setHours(15,00,00);

while(intialTime <= finalTime)
{
    intialTime = new Date(intialTime);
    intialTime = intialTime.setSeconds(intialTime.getSeconds()+1);
    var recordinst = records.find((item)=> 
        item.ts == new Date(intialTime))
    
   if(recordinst == undefined){
       colorsArr.push("r")
   }
   else{
       if(recordinst.machine_status == 1)
       {
           colorsArr.push("yellow")
       }
       if(recordinst.machine_status == 0)
       {
